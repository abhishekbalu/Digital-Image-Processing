...
datapath="C:\Users\Lab User\Downloads\face_recognition\happy";
...
for i=1 : imgcount
    temp = double(X(:,i)) - m;
    A = [A temp];
     subplot(3,5,i),imshow(mat2gray(reshape(A(:,i),r,c)))
end
...
[a,b]=size(eigenfaces);
for i=1:b
    subplot(3,5,i),imshow(mat2gray(reshape(eigenfaces(:,i),r,c)))
end
sgtitle('Eigen faces')
...
for image_index = 1:1:imgcount
    ...
    snr = mean2(uint8(A(:,image_index))./uint8(reconst(:,image_index)));
    subplot(5,6,i),imshow(mat2gray(reshape(m+A(:,image_index), r,c)));
    title('Original Image')
    subplot(25,6,i+1),imshow(mat2gray(reshape(m+reconst(:,image_index),r,c)))
...
testimg= "C:\Users\Lab User\Downloads\face_recognition\6.gif"
...
recognized_img = strcat(datapath,'\',int2str(recognized_index),'.gif');
